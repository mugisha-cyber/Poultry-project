...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM kabere_income_project3 WHERE NO=$id");
    header("Location: view_kabere_income_project3.php");
    exit;
}
...
if (isset($_POST['update'])) {
    ...
    $sql = "UPDATE kabere_income_project3 SET DATE=?, DETAILS=?, AMOUNT=? WHERE NO=?";
...
$result_income = $conn->query("SELECT * FROM kabere_income_project3 ORDER BY DATE DESC");

// Calculate totals
$total_income = 0;
$res = $conn->query("SELECT SUM(AMOUNT) AS total FROM kabere_income_project3");
if ($res && $res->num_rows > 0) {
    $row = $res->fetch_assoc();
    $total_income = $row['total'] ?? 0;
}

// If you have a related expenses table for Kabere
$total_expenses = 0;
$res_exp = $conn->query("SELECT SUM(AMOUNT) AS total FROM kabere_expenses_project3");
...
<head>
    <meta charset="UTF-8">
    <title>Kabere Income Project 3 - Records</title>
    <style>
        body {
...
<h2>Kabere Income Project 3 Records</h2>
