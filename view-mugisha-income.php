...
if (isset($_POST['add'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO mugisha_income (DATE, SOURCE, AMOUNT) VALUES (?, ?, ?)");
    $stmt->bind_param("ssd", $date, $source, $amount);
    $stmt->execute();
    $stmt->close();
    header("Location: mugisha-income.php");
    exit();
}
...
if (isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("UPDATE mugisha_income SET DATE=?, SOURCE=?, AMOUNT=? WHERE NO=?");
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM mugisha_income WHERE NO = $no");
    header("Location: mugisha-income.php");
    exit();
}
...
if (isset($_GET['edit'])) {
    ...
    $edit_query = $conn->query("SELECT * FROM mugisha_income WHERE NO = $edit_id");
    $edit_row = $edit_query->fetch_assoc();
}

// Fetch all records
$result = $conn->query("SELECT * FROM mugisha_income ORDER BY DATE ASC");

// Calculate total income
$total_income = $conn->query("SELECT SUM(AMOUNT) AS total FROM mugisha_income")->fetch_assoc()['total'] ?? 0;
?>
...
<head>
    ...
    <title>MUGISHA Income Records</title>
    <style>
