...
if (isset($_POST['save'])) {
    ...
    if (!empty($date) && !empty($details) && !empty($amount)) {
        $sql = "INSERT INTO `income_of_200` (DATE, DETAILS, AMOUNT) VALUES ('$date', '$details', '$amount')";
        if ($conn->query($sql)) {
...
<head>
    ...
    <title>Add Inkoko 200 Income</title>
    <style>
        ...
        button { padding: 10px 18px; background: #0066cc; color: #fff; border: none; border-radius: 6px; cursor: pointer; transition: 0.3s; }
...
<a href="view_income_200.php">📊 View All Income</a> | 
